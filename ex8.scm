(#%require schemeunit)
(#%require "hanoi.scm")

(define (hanoi-disks-count n step)
  (map-vector length (hanoi-disks n step)))

(define (hanoi-disks n step)
  (define (add vec pos n)
    (case pos
      ((1) (vector (cons n (vector-ref vec 0)) (vector-ref vec 1) (vector-ref vec 2)))
      ((2) (vector (vector-ref vec 0) (cons n (vector-ref vec 1)) (vector-ref vec 2)))
      ((3) (vector (vector-ref vec 0) (vector-ref vec 1) (cons n (vector-ref vec 2))))))
  (define (go n step from to spare vec)
    (if (zero? step)
        (if (zero? n) vec (go (- n 1) 0 from spare to (add vec from n)))
        (let ((mid (expt 2 (- n 1))))
          (if (< step mid)
              (go (- n 1) step from spare to (add vec from n))
              (go (- n 1) (- step mid) spare to from (add vec to n))))))
  (map-vector reverse (go n step 1 2 3 (vector (list) (list) (list)))))

(define (hanoi-picture n step)
  (define (go radii pos height)
    (cond ((null? radii) #f)
          ((null? (cdr radii)) (disk-image (car radii) n pos height))
          (else (above (go (cdr radii) pos (+ height 1)) (disk-image (car radii) n pos height)))))
  (let* ((vec (hanoi-disks n step))
         (pic1 (go (vector-ref vec 0) 1 0))
         (pic2 (go (vector-ref vec 1) 2 0))
         (pic3 (go (vector-ref vec 2) 3 0)))
    (above* (above* (above* pic3 pic2) pic1) (towers-background n))))

(define (above* top bottom)
  (if top (if bottom (above top bottom) top) bottom))

(define (map-vector f vec)
  (list->vector (map f (vector->list vec))))

(check-equal? (hanoi-disks-count 3 0) (vector 3 0 0))
(check-equal? (hanoi-disks-count 3 1) (vector 2 1 0))
(check-equal? (hanoi-disks-count 3 2) (vector 1 1 1))
(check-equal? (hanoi-disks-count 3 3) (vector 1 0 2))
(check-equal? (hanoi-disks-count 3 4) (vector 0 1 2))
(check-equal? (hanoi-disks-count 3 5) (vector 1 1 1))
(check-equal? (hanoi-disks-count 3 6) (vector 1 2 0))
(check-equal? (hanoi-disks-count 3 7) (vector 0 3 0))
(check-equal? (hanoi-disks-count 5 13) (vector 2 1 2))
(check-equal? (hanoi-disks-count 15 19705) (vector 2 9 4))
(check-equal? (hanoi-disks-count 15 32767) (vector 0 15 0))

(check-equal? (hanoi-disks 5 0) (vector (list 5 4 3 2 1) (list) (list)))
(check-equal? (hanoi-disks 5 13) (vector (list 5 2) (list 1) (list 4 3)))
(check-equal? (hanoi-disks 5 22) (vector (list 3 2) (list 5) (list 4 1)))
(check-equal? (hanoi-disks 5 31) (vector (list) (list 5 4 3 2 1) (list)))
(check-equal? (hanoi-disks 15 19705)
              (vector (list 3 2) (list 15 12 11 8 7 6 5 4 1) (list 14 13 10 9)))
